...
class ACM(nn.Module):
    ...
    def forward(self, x):
        ...
        add_feature, add_att_maps = self.add_mod(x_mu)    # K
        # creates add or sub feature
        sub_feature, sub_att_maps = self.sub_mod(x_mu)    # Q
...
class AttendModule(nn.Module):
    ...
    def batch_weighted_avg(self, x, weights):
        b, c, h, w = x.shape
        # xhat reshape
        x_reshape = x.view(b * self.num_heads, self.num_c_per_head, h, w)
        x_reshape = x_reshape.view(b * self.num_heads, self.num_c_per_head, h * w)
        ...
        mus = torch.bmm(x_reshape, weights_normalized)
...
if __name__ == '__main__':
    a = torch.zeros((5, 256, 20, 20))
    b = torch.ones((5, 256, 1, 1))
